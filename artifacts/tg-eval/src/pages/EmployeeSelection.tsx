...
export default function EmployeeSelection() {
  ...
  // Оценщик — это вошедший пользователь (авторизация = выбор оценщика)
  useEffect(() => {
    if (currentUser && (!selectedEvaluator || selectedEvaluator.id !== currentUser.id)) {
  ...
  }, [currentUser, selectedEvaluator, setSelectedEvaluator]);
  ...
  return (
    ...
    >
      ...
      <header className="sticky top-0 z-10 px-4 pt-12 pb-3" style={{ background: "rgba(242,242,247,0.92)", backdropFilter: "blur(20px)", WebkitBackdropFilter: "blur(20px)" }}>
        ...
        <h1 className="text-[28px] font-bold mb-1" style={{ color: "#000", letterSpacing: "-0.3px" }}>
          Кого оцениваем?
        </h1>
        {currentUser && (
          <p className="text-[14px] mb-3" style={{ color: "rgba(60,60,67,0.6)" }}>
            Оценщик: <span style={{ color: "#007AFF", fontWeight: 600 }}>{currentUser.name}</span>
          </p>
        )}
